    ...
    try {
      ...
      if (
        ...
        return {
          success: false,
          error:
            result.message || result.error || "Unknown error from SlipOK API",
          data: result.data || null,
        };
      }
    } catch (axiosError) {
      console.log(
        "⚠️ Axios failed, trying fetch method...",
        axiosError.message
      );
    }
 ...
 */
export const verifySlipWithSlipOKSimple = async (
  fileBuffer,
  fileName,
  mimeType
) => {
  try {
    ...
    console.log(
      "🔍 Calling SlipOK API (Simple Method):",
      fileName,
      `(${fileBuffer.length} bytes)`
    );
...
export const parseSlipResult = (slipResult) => {
  try {
    console.log(
      "🔍 Parsing SlipOK result:",
      JSON.stringify(slipResult, null, 2)
    );

    if (!slipResult.success) {
      const errorMessage =
        slipResult.message || slipResult.error || "ไม่สามารถอ่านสลิปได้";
    ...
    const parsedData = {
      // ข้อมูลพื้นฐาน
      amount:
        parseFloat(data.amount || data.total || data.transferAmount || 0) ||
        null,
      date:
        data.date ||
        data.transDate ||
      ...
      // ข้อมูลผู้โอน
      senderAccount:
        data.sender?.account || data.fromAccount || data.senderAccount || null,
      ...
      receiverAccount:
        ...
        null,
      receiverName:
        data.receiver?.name || data.toName || data.receiverName || null,
      receiverBank:
        data.receiver?.bank || data.toBank || data.receiverBank || null,

      // ข้อมูลเพิ่มเติม
      transactionRef:
        data.ref || data.ref1 || data.transactionId || data.refNo || null,
  ...
  try {
    ...
    if (slipData.date) {
      try {
        ...
        const daysDiffFromOrder = Math.abs(
          (slipDate - orderDate) / (1000 * 60 * 60 * 24)
        );
        const daysDiffFromToday = Math.abs(
          (slipDate - today) / (1000 * 60 * 60 * 24)
        );
        ...
        if (daysDiffFromOrder <= 7 && daysDiffFromToday <= 1) {
          ...
          validations.push({
            type: "date",
            status: "warning",
            message: `วันที่โอนเงินห่างจากวันสั่งซื้อ ${Math.round(
              daysDiffFromOrder
            )} วัน`,
          });
        }
    ...
    if (slipData.confidenceScore !== null && slipData.confidenceScore < 0.8) {
      warnings.push({
        type: "confidence",
        message: `ความเชื่อมั่นในการอ่านสลิปต่ำ: ${Math.round(
          slipData.confidenceScore * 100
        )}%`,
      });
    }
